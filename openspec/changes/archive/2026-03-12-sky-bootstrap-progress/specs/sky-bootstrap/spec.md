...
Sistema de bootstrap com barra de carregamento e feedback visual de componentes sendo carregados durante a inicialização do Sky Chat.
...
```
core/sky/bootstrap/
├── __init__.py          # Exports: run, Stage, Progress
├── bootstrap.py         # Orquestrador: def run() -> SkyApp
...
app = run()  # Mostra barra de progresso, retorna SkyApp
app.run()
```
...
```python
STAGES = [
    Stage("environment", "Configurando ambiente...", 0.1),
    Stage("embedding", "Carregando modelo de embedding...", 10.0),  # Peso maior
    Stage("vector-db", "Inicializando banco vetorial...", 1.0),
    Stage("collections", "Configurando coleções...", 0.5),
    Stage("textual", "Iniciando interface...", 0.5),
]
```

### Exit Codes

- `0`: Success, Sky Chat started
- `1`: Error during bootstrap (missing dependency, corrupted DB)
