...
- `valor: Decimal`
- `currency: Optional[str]` (padrão: "BRL")

#### Scenario: Depósito em BRL aumenta saldo
- **WHEN** handler processa command com valor=10000, currency="BRL"
- **THEN** cashbook["entries"]["BRL"]["amount"] aumenta em 10000
- **AND** paper_state é atualizado

#### Scenario: Depósito em USD adiciona nova moeda
- **WHEN** handler processa command com valor=1000, currency="USD"
- **THEN** cashbook["entries"]["USD"] é criada com amount=1000
- **AND** conversion_rate é obtida via CurrencyConverterPort
